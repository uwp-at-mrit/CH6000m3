...
template<class G>
static void _DI_gantry(G* target, const uint8* db4, unsigned int idx4_p1, const uint8* db205, unsigned int idx205_p1) {
	if (DI_gantry_pushed_out(db4, idx4_p1)) {
		target->set_state(GantryState::PushedOut);
	} else if (DI_gantry_pulled_in(db4, idx4_p1)) {
		target->set_state(GantryState::PulledIn);
	} else if (DI_gantry_pushing_out(db205, idx205_p1)) {
		target->set_state(GantryState::PushingOut);
	} else if (DI_gantry_pulling_in(db205, idx205_p1)) {
...
bool WarGrey::SCADA::DI_gantry_pushing_out(const uint8* db205, unsigned int idx_p1) {
	return DBX(db205, idx_p1 - 1U);
}

bool WarGrey::SCADA::DI_gantry_pulling_in(const uint8* db205, unsigned int idx_p1) {
	return DBX(db205, idx_p1 + 0U);
}

bool WarGrey::SCADA::DI_gantry_moving(const uint8* db205, unsigned int idx_p1) {
	return DI_gantry_pushing_out(db205, idx_p1) || DI_gantry_pulling_in(db205, idx_p1);
}

bool WarGrey::SCADA::DI_compensator_working(const uint8* db205, unsigned int idx_p1) {
	return DBX(db205, idx_p1 - 1U) || DBX(db205, idx_p1 + 0U);
}

bool WarGrey::SCADA::DI_visor_curling(const uint8* db205, unsigned int idx_p1) {
	return DBX(db205, idx_p1 - 1U) || DBX(db205, idx_p1 + 0U);
}
