...
static TableColumnInfo track_columns[] = {
    ...
    { "suction", SDT::Float, nullptr, 0 | DB_NOT_NULL | 0 },
    { "timestamp", SDT::Integer, nullptr, 0 | DB_NOT_NULL | DB_UNIQUE },
};

/**************************************************************************************************/
EarthWork_pk WarGrey::SCADA::track_identity(EarthWork& self) {
    return self.uuid;
}

EarthWork WarGrey::SCADA::make_track(std::optional<Integer> group, std::optional<Float> x, std::optional<Float> y, std::optional<Float> z, std::optional<Float> suction, std::optional<Integer> timestamp) {
    EarthWork self;

    default_track(self, group, x, y, z, suction, timestamp);

    return self;
}

void WarGrey::SCADA::default_track(EarthWork& self, std::optional<Integer> group, std::optional<Float> x, std::optional<Float> y, std::optional<Float> z, std::optional<Float> suction, std::optional<Integer> timestamp) {
    ...
    if (suction.has_value()) { self.suction = suction.value(); }
    if (timestamp.has_value()) { self.timestamp = timestamp.value(); }
}

void WarGrey::SCADA::refresh_track(EarthWork& self) {
}

void WarGrey::SCADA::store_track(EarthWork& self, IPreparedStatement* stmt) {
    ...
    stmt->bind_parameter(5U, self.suction);
    stmt->bind_parameter(6U, self.timestamp);
}

void WarGrey::SCADA::restore_track(EarthWork& self, IPreparedStatement* stmt) {
    ...
    self.suction = stmt->column_double(5U);
...
void WarGrey::SCADA::insert_track(IDBSystem* dbc, EarthWork& self, bool replace) {
    insert_track(dbc, &self, 1, replace);
}

void WarGrey::SCADA::insert_track(IDBSystem* dbc, EarthWork* selves, size_t count, bool replace) {
    IVirtualSQL* vsql = dbc->make_sql_factory(track_columns);
...
void WarGrey::SCADA::foreach_track(IDBSystem* dbc, IEarthWorkCursor* cursor, uint64 limit, uint64 offset, track order_by, bool asc) {
    IVirtualSQL* vsql = dbc->make_sql_factory(track_columns);
...
std::list<EarthWork> WarGrey::SCADA::select_track(IDBSystem* dbc, uint64 limit, uint64 offset, track order_by, bool asc) {
    IVirtualSQL* vsql = dbc->make_sql_factory(track_columns);
    const char* colname = ((order_by == track::_) ? nullptr : track_columns[static_cast<unsigned int>(order_by)].name);
    std::string sql = vsql->select_from("track", colname, asc, limit, offset);
    IPreparedStatement* stmt = dbc->prepare(sql);
...
std::optional<EarthWork> WarGrey::SCADA::seek_track(IDBSystem* dbc, EarthWork_pk where) {
    IVirtualSQL* vsql = dbc->make_sql_factory(track_columns);
...
void WarGrey::SCADA::update_track(IDBSystem* dbc, EarthWork& self, bool refresh) {
    update_track(dbc, &self, 1, refresh);
}

void WarGrey::SCADA::update_track(IDBSystem* dbc, EarthWork* selves, size_t count, bool refresh) {
    IVirtualSQL* vsql = dbc->make_sql_factory(track_columns);
    ...
    if (stmt != nullptr) {
        for (size_t i = 0; i < count; i ++) {
            ...
            stmt->bind_parameter(4U, selves[i].suction);
...
void WarGrey::SCADA::delete_track(IDBSystem* dbc, EarthWork_pk& where) {
    delete_track(dbc, &where, 1);
}

void WarGrey::SCADA::delete_track(IDBSystem* dbc, EarthWork_pk* wheres, size_t count) {
    IVirtualSQL* vsql = dbc->make_sql_factory(track_columns);
